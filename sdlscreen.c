...
void GFX2_Set_mode(int *width, int *height, int fullscreen)
{
  ...
  if (Window_SDL == NULL)
  {
    ...
    Renderer_SDL = SDL_CreateRenderer(Window_SDL, 
                                      -1,
                                      0);
  }
  else
  ...
  Texture_SDL = SDL_CreateTexture(Renderer_SDL,
...
static void GFX2_UpdateRect(int x, int y, int width, int height)
{
  ...
  {
      memcpy(pixels + line * pitch,
              (const byte *)RGBcopy->pixels +
              source_rect.x * 4 +
              (source_rect.y+line)* RGBcopy->pitch,
              source_rect.w * 4);
  }
